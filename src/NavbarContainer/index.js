...
export default function Navbar(props) {
  console.log(props.userCart.length, 'here is the cart info from navBar');
  return (
    <div>
        ...
        <nav className='nav-bar'>
          <ul>
            ...
            <li className='nav-bar-item'><a name='cart' onClick={props.navChange} href='#'><img className='shopping-cart' src={shoppingCart} alt={'shopping cart image'}/><span>{props.userCart.length > 0 ? props.userCart.length : null}</span></a></li>
          </ul>
        </nav>
    </div>
  );
